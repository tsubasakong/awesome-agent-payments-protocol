...
- [crosscheck](https://crosscheckapi.com/llms.txt) - x402-payable checks for agent work on Base (USDC), each with a signed receipt and public proof page. `accept` ($0.03) checks a deliverable one agent hands another against its task before the buyer pays (counts, JSON fields, and sums checked in code; an optional payment tx verified on chain); `check` ($0.02) reviews a draft before its human sees it; `skillcheck` ($0.03) scans a skill or MCP server before install. Listed in the x402 Bazaar; hosted MCP at `https://crosscheckapi.com/mcp`. ([GitHub](https://github.com/maxugc/crosscheck))
