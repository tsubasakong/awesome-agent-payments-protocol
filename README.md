...
- [Spraay x402 Gateway](https://gateway.spraay.app) — Multi-chain batch payment gateway: pay up to 200 recipients in one atomic USDC transaction on Base, with batch rails also on Stellar and XRP Ledger. Dual-rail **x402** (USDC on Base/Solana) + **MPP** (pathUSD on Tempo) across 190 endpoints, free validate/estimate before paid execution, non-custodial. Includes an MCP server (114 tools). ([x402 manifest](https://gateway.spraay.app/.well-known/x402.json), [BPA 1.0 spec](https://docs.spraay.app/bpa/1.0/))
