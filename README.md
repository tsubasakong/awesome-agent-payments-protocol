...
- [AVEL](https://github.com/Ensi81/Avel) — Verification layer for agent-to-agent payments: checks whether payer and payee are real, identified agents with a reputation built from real recorded outcomes (never self-reported), across x402/AP2/MPP/checkout. `POST /verify` returns a verdict + both parties' reputation; never custodies or moves the payment. Python SDK (MIT) + MCP server.
