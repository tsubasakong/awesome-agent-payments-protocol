...
- [sigtap](https://sigtap-outreach-api.sigtap.workers.dev) — x402-payable agent micro-tools on Base: email-outreach toolkit (deliverability audit SPF/DKIM/DMARC/MX, cold-email grader, template generator) + agent utilities (JWT decode, UUIDv7/ULID, slug, regex, JSON format/validate, domain age, crypto price, weather). $0.0005–$0.01 USDC per call, no API keys; every paid route has a free `/preview/<path>` sample. ([MCP](https://sigtap-mcp.sigtap.workers.dev/mcp), [OpenAPI](https://sigtap-outreach-api.sigtap.workers.dev/openapi.json), [llms.txt](https://sigtap-outreach-api.sigtap.workers.dev/llms.txt), [PyPI](https://pypi.org/project/sigtap-agent-tools/))
...
- **NPCI / UPI (India)** — **Unified Agent Protocol** (unveiling expected Sep 9–11, 2026 at Global Fintech Fest): agents registered/verified to transact on UPI within user-set limits without per-transaction approval, built on UPI Circle delegation — first national payment infrastructure with a native agentic rail ([Reuters](https://www.brecorder.com/news/40437409/india-preparing-rollout-of-agentic-payments-on-upi-sources-say))
