...
- [SpendShield](https://github.com/felixpg13-glitch/spendshield) — Policy & authorization layer for agent payments: ALLOW / APPROVAL / DENY with budgets, per-transaction caps, merchant allow/block lists, signed one-time grants and tamper-evident audit. Python library with an x402 adapter (`protect_x402_payment` / `X402PaywallGuard`) and an MCP server.
