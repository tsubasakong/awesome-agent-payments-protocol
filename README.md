...
- [Animica](https://animica.dev/x402) — 44 x402-payable APIs on Base, settled by a **self-hosted EIP-3009 facilitator (no CDP)**. Every 402 offers two lanes: `eip155:8453` (USDC on Base) and `animica:1` (**native ANM at a standing 25% discount** — the payer submits their own transaction, so no gas is sponsored and the saving is passed on). Includes **Paid Crawl**, where a website charges AI crawlers per page: free for site owners with no account, enforced by a free unmetered decision endpoint that fails open, with crawlers buying bulk passes and claiming a **post-quantum ML-DSA-65 (FIPS 204) licence** proving what they licensed — free to claim and free for anyone to verify. Also a GEO/agent-legibility audit, post-quantum signature verification, on-chain notarisation, verifiable randomness, and OpenAI-compatible inference. Prepaid credits turn one settlement into many gas-free calls. ([OpenAPI](https://animica.dev/x402/openapi.json), [Discovery](https://animica.dev/.well-known/x402), [Paid Crawl protocol](https://animica.dev/.well-known/paid-crawl), [GitHub](https://github.com/animicaorg/all))
